...
{ ------------------------------------------------------------ }
 ...
 const NOUN_ru: integer = 9;                       { class —”Ÿ≈—“¬»“≈À‹ÕŒ≈ }
 const PRONOUN2_ru: integer = 10;                  { class Ã≈—“Œ»Ã_—”Ÿ }
 const PRONOUN_ru: integer = 11;                   { class Ã≈—“Œ»Ã≈Õ»≈ }
 const ADJ_ru: integer = 12;                       { class œ–»À¿√¿“≈À‹ÕŒ≈ }
 const NUMBER_CLASS_ru: integer = 13;              { class ◊»—À»“≈À‹ÕŒ≈ }
 const INFINITIVE_ru: integer = 14;                { class »Õ‘»Õ»“»¬ }
 const VERB_ru: integer = 15;                      { class √À¿√ŒÀ }
 const GERUND_2_ru: integer = 16;                  { class ƒ≈≈œ–»◊¿—“»≈ }
 const PREPOS_ru: integer = 17;                    { class œ–≈ƒÀŒ√ }
 const IMPERSONAL_VERB_ru: integer = 18;           { class ¡≈«À»◊_√À¿√ŒÀ }
 const PARTICLE_ru: integer = 21;                  { class ◊¿—“»÷¿ }
 const CONJ_ru: integer = 22;                      { class —Œﬁ« }
 const ADVERB_ru: integer = 23;                    { class Õ¿–≈◊»≈ }
 const PUNCTUATION_class: integer = 24;            { class œ”Õ “”¿“Œ– }
...
const FUTURE_ru: integer = 2;                      // ¬–≈Ãﬂ : ¡”ƒ”Ÿ≈≈
 ...
 const VOICE_ru: integer = 23;                     { enum «¿ÀŒ√ }
{ Coordiname «¿ÀŒ√ states: }
const PASSIVE_VOICE_ru: integer = 0;               // «¿ÀŒ√ : —“–¿ƒ
const ACTIVE_VOICE_ru: integer = 1;                // «¿ÀŒ√ : ƒ≈…—“¬
 const CASE_ru: integer = 26;                      { enum œ¿ƒ≈∆ }
...
const SUPERLATIVE_FORM_ru: integer = 2;            // —“≈œ≈Õ‹ : œ–≈¬Œ—’
 const CASE_GERUND_ru: integer = 30;               { enum œ‡‰ÂÊ¬‡Î }
{ Coordiname œ‡‰ÂÊ¬‡Î states: }
 const MODAL_ru: integer = 31;                     { enum ÃŒƒ¿À‹Õ€… }
 const VERBMODE_TENSE: integer = 32;               { enum —√ƒ_¬ÂÏˇ }
{ Coordiname —√ƒ_¬ÂÏˇ states: }
 const VERBMODE_DIRECTION: integer = 33;           { enum —√ƒ_Õ‡Ô‡‚ÎÂÌËÂ }
{ Coordiname —√ƒ_Õ‡Ô‡‚ÎÂÌËÂ states: }
 const TENSE_en: integer = 34;                     { enum TENSE }
...
const FUTURE_en: integer = 2;                      // TENSE : FUTURE
 const DURATION_en: integer = 35;                  { enum DURATION }
...
const PREPOSITIVE_CASE_en: integer = 1;            // CASE : PREPOSITIVE
 const NOUN_FORM_en: integer = 38;                 { enum NOUN_FORM }
...
const POSSESSIVE_NOUN_FORM_en: integer = 1;        // NOUN_FORM : POSSESSIVE
 const PRONOUN_FORM_en: integer = 39;              { enum PRONOUN_FORM }
{ Coordiname PRONOUN_FORM states: }
 const ADJ_FORM_en: integer = 40;                  { enum ADJ_FORM }
...
const SUPERLATIVE_ADJ_en: integer = 2;             // ADJ_FORM : SUPERLATIVE
 const COMPARABILITY_en: integer = 41;             { enum COMPARABILITY }
...
const NONCOMPARABLE: integer = 3;                  // COMPARABILITY : NONCOMPARABLE
 const VERB_FORM_en: integer = 42;                 { enum VERB_FORM }
...
const TO_MODALITY_en: integer = 2;                 // ENG_MODALITY : To
 const NUMERAL_FORM_en: integer = 45;              { enum NUMERAL_FORM }
...
const ORDINAL_en: integer = 1;                     // NUMERAL_FORM : ORDINAL
 const GENDER_en: integer = 46;                    { enum ENG_GENDER }
...
const FEMININE_en: integer = 1;                    // ENG_GENDER : FEMININE
 const TRANSITIVITY_en: integer = 47;              { enum TRANSITIVITY }
...
const TRANSITIVE_VERB_en: integer = 1;             // TRANSITIVITY : TRANSITIVE
 const OBLIG_TRANSITIVITY_en: integer = 48;        { enum OBLIG_TRANSITIVITY }
 const PROPER_NOUN_en: integer = 49;               { enum ENG_PROPER_NOUN }
 const MASS_NOUN_en: integer = 50;                 { enum ENG_MASS_NOUN }
 const PERSON_fr: integer = 51;                    { enum FR_PERSON }
...
const ORDINAL_fr: integer = 1;                     // FR_NUMERAL_FORM : ORDINAL
 const FR_PRONOUN_FORM: integer = 55;              { enum FR_PRONOUN_FORM }
...
const FR_PRONOUN_STRONG: integer = 1;              // FR_PRONOUN_FORM : STRONG
 const TRANSITIVITY_fr: integer = 56;              { enum FR_TRANSITIVITY }
...
const TRANSITIVE_VERB_fr: integer = 1;             // FR_TRANSITIVITY : TRANSITIVE
 const VERB_FORM_fr: integer = 57;                 { enum FR_VERB_FORM }
...
const IMPERATIVE_fr: integer = 10;                 // FR_VERB_FORM : IMPERATIVE
 const JAP_FORM: integer = 58;                     { enum JAP_FORM }
...
const ROMAJI_FORM: integer = 2;                    // JAP_FORM : ROMAJI
 const JAP_VERB_BASE: integer = 59;                { enum JAP_VERB_BASE }
...
const JAP_VB_CAUSATIVE: integer = 9;               // JAP_VERB_BASE : CAUSATIVE
 const JAP_VERB_KIND: integer = 60;                { enum JAP_VERB_KIND }
...
const JAP_NEGATIVE_IMPERATIVE: integer = 6;        // JAP_VERB_KIND : NEGATIVE_IMPERATIVE
 const JAP_ADJ_BASE: integer = 61;                 { enum JAP_ADJ_BASE }
...
const JAP_AB_PAST: integer = 6;                    // JAP_ADJ_BASE : PAST
 const JAP_ADJ_FORM2: integer = 62;                { enum JAP_ADJ_FORM2 }
...
const JAP_NEGATIVE_PAST_ADJ: integer = 1;          // JAP_ADJ_FORM2 : NEGATIVE_PAST
 const JAP_TRANSITIVE: integer = 63;               { enum JAP_TRANSITIVE }
 const CASE_jap: integer = 64;                     { enum JAP_CASE }
...
const SOCIATIVE_jap: integer = 12;                 // JAP_CASE : SOCIATIVE
 const GENDER_jap: integer = 65;                   { enum JAP_GENDER }
...
const FEMININE_jap: integer = 1;                   // JAP_GENDER : FEMININE
 const PERSON_jap: integer = 66;                   { enum JAP_PERSON }
...
{ ------------------------------------------------------------ }
 ...
 const LOGIC_ITEM_link: integer = 7;
 const SUBJECT_link: integer = 8;
 const RHEMA_link: integer = 9;
 const COVERB_link: integer = 10;
 const NUMBER2OBJ_link: integer = 16;
 const TENSE_VERB_link: integer = 31;
 const JAP_COMITATIVE_link: integer = 33;
 const JAP_COMITATIVE2_link: integer = 34;
 const TO_VERB_link: integer = 35;
 const TO_INF_link: integer = 36;
 const TO_PERFECT: integer = 37;
 const TO_UNPERFECT: integer = 38;
 const TO_NOUN_link: integer = 39;
 const TO_ADJ_link: integer = 40;
 const TO_ADV_link: integer = 41;
 const TO_RETVERB: integer = 42;
 const TO_GERUND_2_link: integer = 43;
 const WOUT_RETVERB: integer = 44;
 const TO_ENGLISH_link: integer = 45;
 const TO_RUSSIAN_link: integer = 46;
 const TO_FRENCH_link: integer = 47;
 const SYNONYM_link: integer = 48;
 const SEX_SYNONYM_link: integer = 49;
 const CLASS_link: integer = 50;
 const MEMBER_link: integer = 51;
 const TO_SPANISH_link: integer = 52;
 const TO_GERMAN_link: integer = 53;
 const TO_CHINESE_link: integer = 54;
 const TO_POLAND_link: integer = 55;
 const TO_ITALIAN_link: integer = 56;
 const TO_PORTUGUAL_link: integer = 57;
 const ACTION_link: integer = 58;
 const ACTOR_link: integer = 59;
 const TOOL_link: integer = 60;
 const RESULT_link: integer = 61;
 const TO_JAPANESE_link: integer = 62;
 const TO_KANA_link: integer = 63;
 const TO_KANJI_link: integer = 64;
 const ANTONYM_link: integer = 65;
 const EXPLANATION_link: integer = 66;
 const WWW_link: integer = 67;
 const ACCENT_link: integer = 68;
 const YO_link: integer = 69;
 const TO_DIMINUITIVE_link: integer = 70;
 const TO_RUDE_link: integer = 71;
 const TO_BIGGER_link: integer = 72;
 const TO_NEUTRAL_link: integer = 73;
 const TO_SCOLARLY: integer = 74;
 const TO_SAMPLE_link: integer = 75;
 const USAGE_TAG_link: integer = 76;
 const DOUBLE_LETTERS_link: integer = 77;
 const PROPERTY_link: integer = 78;
 const TO_CYRIJI_link: integer = 79;
 const HABITANT_link: integer = 80;
 const NGRAM_ETALON: integer = 81;
 const CHILD_link: integer = 82;
 const PARENT_link: integer = 83;
 const UNIT_link: integer = 84;
 const SET_link: integer = 85;
 const TO_WEAKENED_link: integer = 86;
 const SUBSTITUTION_link: integer = 87;
 const VERBMODE_BASIC_link: integer = 88;
 const NEGATION_PARTICLE_link: integer = 89;
 const NEXT_COLLOCATION_ITEM_link: integer = 90;
 const VIRTUAL_NODE_CONTENT_link: integer = 91;
 const SUBORDINATE_CLAUSE_link: integer = 92;
 const RIGHT_GENITIVE_OBJECT_link: integer = 93;
 const ADV_PARTICIPLE_link: integer = 94;
 const POSTFIX_PARTICLE_link: integer = 95;
 const INFINITIVE_link: integer = 96;
 const NEXT_ADJECTIVE_link: integer = 97;
 const NEXT_NOUN_link: integer = 98;
 const THEMA_link: integer = 99;
 const RIGHT_PARTICLE_link: integer = 100;
 const RIGHT_AUX2INFINITIVE_link: integer = 101;
 const RIGHT_AUX2PARTICIPLE: integer = 102;
 const RIGHT_AUX2ADJ: integer = 103;
 const RIGHT_LOGIC_ITEM_link: integer = 104;
 const RIGHT_COMPARISON_Y_link: integer = 105;
 const RIGHT_NOUN_link: integer = 106;
 const RIGHT_INSTR_OBJECT_link: integer = 107;
 const RIGHT_DATIVE_OBJECT_link: integer = 108;
 const RIGHT_NAME_link: integer = 109;
 const ADJ_PARTICIPLE_link: integer = 110;
 const RIGHT_ADJ_CLAUSE_link: integer = 111;
 const PUNCTUATION_link: integer = 112;
 const IMPERATIVE_SUBJECT_link: integer = 113;
 const IMPERATIVE_VERB2AUX_link: integer = 114;
 const AUX2IMPERATIVE_VERB: integer = 115;
 const PREFIX_PARTICLE_link: integer = 116;
 const ADRESS_TO_LISTENER_link: integer = 117;
 const PREFIX_CONJUNCTION_link: integer = 118;
 const LOGICAL_CONJUNCTION_link: integer = 119;
 const NEXT_CLAUSE_link: integer = 120;
 const LEFT_AUX_VERB_link: integer = 121;
 const BEG_INTRO_link: integer = 122;
 const RIGHT_PREPOSITION_link: integer = 123;
 const WH_SUBJECT_link: integer = 124;
 const IMPERATIVE_PARTICLE_link: integer = 125;
 const GERUND_link: integer = 126;
 const PREPOS_ADJUNCT_link: integer = 127;
 const DIRECT_OBJ_INTENTION_link: integer = 128;
 const COPULA_link: integer = 129;
 const DETAILS_link: integer = 130;
 const SENTENCE_CLOSER_link: integer = 131;
 const OPINION_link: integer = 132;
 const APPEAL_link: integer = 133;
 const TERM_link: integer = 134;
 const SPEECH_link: integer = 135;
 const QUESTION_link: integer = 136;
 const POLITENESS_link: integer = 137;
implementation
end.
