...
static int CheckNode( HGREN hEngine, HGREN_TREENODE hNode, const char *line, bool write_errors, const char *sentence_utf8, HGREN_RESPACK hs )
{
 int nerr=0;
     ...
     for( int idim=1; idim<toks.size(); ++idim )
      {
       ...
       if( bistable_state==0 )
        ...
        {
         state_matched = sol_GetNodeCoordPair( hNode, CategoryID, StateID )!=-1;
        }
...
static int CheckTree( HGREN hEngine, HGREN_TREENODE hNode, FILE *file, bool write_errors, const char *sentence_utf8, HGREN_RESPACK hs )
{
 int nerr=0;
 ...
 nerr = CheckNode( hEngine, hNode, line, write_errors, sentence_utf8, hs );
 if( nerr==0 )
  {
   ...
   if( nleaf>0 )
    {
     ...
     for( int i=0; i<nleaf; i++ )
      {
       nerr += CheckTree( hEngine, sol_GetLeaf( hNode, i ), file, write_errors, sentence_utf8, hs );
      }
...
void TestMorphologicalAnalyzer( HGREN hEngine, int id_language, const char *filename, bool write_errors, int thread_no )
{
 ...
 while( !feof(file) )
   ...
   *line=0;
   ...
   if( line[0]=='#' )
    {
     ...
     else if( memcmp( line, "#iterative", 10 )==0 )
      {
       EnableIterative=true;
       EnableSparse=false;
      }
     else if( memcmp( line, "#solid", 14 )==0 )
      {
       EnableSparse=false;
       EnableIterative=false;
      }
   ...
   if( EnableTokenReconstruction ) Flags |= SOL_GREN_ENABLE_RECONSTRUCTION;

   HGREN_RESPACK hs = sol_MorphologyAnalysis8( hEngine, line, Flags, 0, 300000, id_language );
   ...
   for( int i=1; i<nroot-1; ++i )
    {
     ...
     reads( line, 1000, file );
     nerr += CheckNode( hEngine, hNode, line, write_errors, sentence, hs );
    }
...
void TestSyntacticAnalyzer( HGREN hEngine, int id_language, const char *filename, bool write_errors, int thread_no )
{
 ...
 while( !feof(file) )
   ...
   *line=0;
   ...
   if( EnableTokenReconstruction ) MorphologicalFlags |= SOL_GREN_ENABLE_RECONSTRUCTION;
   if( EnableSparse ) SyntacticFlags |= SOL_GREN_SPARSE;
   if( EnableIterative ) SyntacticFlags |= SOL_GREN_ITERATIVE; 
             ...
             for( int i=1; i<nroot-1; ++i )
              {
               ...
               HGREN_TREENODE hNode = sol_GetRoot( hPack, imin_graf, i );
               int err = CheckTree( hEngine, hNode, file, write_errors, sentence, hPack );
               if( err!=0 )
                {
                 ok=false;
                 break;
