...
void failed( HFAIND hEngine )
{
 ...
 sol_GetError( hEngine, err_text, 1000 );
...
bool TestSyntaxAnalysis( HFAIND hEngine, const wchar_t *Phrase, int Language, bool must_be_complete_analysis )
{
...
void Print_StrList( HFAIND hEngine, HGREN_STR hStr )
{
...
void Print_IntList( HFAIND hEngine, HGREN_INTARRAY hList )
{
 if( hList!=NULL )
  {
   wchar_t buffer[64];
...
// *************************************************************************
bool TestLink( HFAIND hEngine, const wchar_t *Word1, int Class1, const wchar_t *Word2, int Class2 )
{
 bool ok = false;
...
bool ProbeRussian( HFAIND hEngine )
{
...
bool ProbeEnglish( HFAIND hEngine )
{
 const int MAX_LEN = sol_MaxLexemLen(hEngine);
