...
struct ThreadRes
{
 ...
 infile_morph(NULL), infile_syntax(NULL), new_syntactic_analyzer(false) {}
...
#endif
{
 ...
 HFAIND hEngine = res->hEngine;

 if( !(res->ithread%2) )
  {
   TestMorphologicalAnalyzer( hEngine, res->id_language, res->infile_morph, false, res->ithread );
   TestSyntacticAnalyzer( hEngine, res->id_language, res->infile_syntax, false, res->ithread );
  }
 else
  {
   TestSyntacticAnalyzer( hEngine, res->id_language, res->infile_syntax, false, res->ithread );
